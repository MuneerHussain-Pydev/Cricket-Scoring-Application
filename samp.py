...
bat_img=CTkImage(light_image=batsman_img,size=(125,125))
bat_lab=CTkLabel(f2,width=60,height=90,text=" ",image=bat_img).place(x=100,y=13)
...
team_name1.bind('<Return>',toss)
team_name2.bind('<Return>',toss)
opt.bind('<Return>',toss)

def batter_name(bat_name_1,bat_name_2,batting):
    #rhis function just inserts batsmen names in tables
...
def next_screen():
    ...
    messagebox.showinfo("MATCH HAS STARTED")
...
def innings_change2():
    ...
    t1_lab=CTkLabel(score_f,text=target+1,fg_color="white",font=("Times",18),text_color="darkGreen",corner_radius=40)
...
def match_finish():
    ...
    if score>target:
        ...
        if bowling_side==team_a_bowling:
            ...
            winner = team_b
    messagebox.showinfo("MATCH ENDED",f"Match won by {winner}")
    h.destroy()
    summary()
...
def undo():
    ...
    if recent[0]=="DOT":
        ...
        messagebox.showerror("ERROR","UNDO function can not work on 'OUT'")
    elif recent[0]=="NB":
        global f1
        BALLS+=1
        X= BALLS//6
